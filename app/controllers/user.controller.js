...
const createNewUser = (req, res) => {
    ...
    const userObj = new User({
        ...
        img: req.body.img
    });  // salt = random number or random language
    User.create(userObj, (err, data) => {
        if(err){
            res.status(500).send({ message: err.message || "Some err occured while creating"})
        }
        else res.send(data);
    });
};
...
const login = (req, res) =>{
    ...
    const acc = new User({
        username: req.body.username,               /// account
...
const updateUserCtrl = (req, res)=>{
    ...
    // console.log("userctrl"+JSON.stringify(data))
...
const deleteUser = (req, res)=>{
    console.log("parameters" + req.params.id + ", " + req.params.p1 + ", " + req.params.p1); // parameters check
...
const getUserRecipeCount = (req, res) => {
    User.getUserRecipeCount((err, data) => {
      if (err) {
        res.status(500).send({
...
module.exports = { validUsername, createNewUser, login, getAllUsers,updateUserCtrl,deleteUser, getUserRecipeCount};
